...
MANIFESTS_URL?="git@git.kaiostech.com:KaiOS/manifests.git"
...
# See Bug 11741:
# https://bugzilla.kaiostech.com/show_bug.cgi?id=11741
ifeq ($(SYS),Darwin)
...
GAIA_ALLAPPDIRS=$(shell find -L $(GAIA_DIR)$(SEP)apps -maxdepth 1 -mindepth 1 -type d 2>/dev/null  | sed 's@[/\\]@$(SEP_FOR_SED)@g')
...
# Generate profile/
$(PROFILE_FOLDER): profile-dir app contacts extensions .git/hooks/pre-commit
ifeq ($(BUILD_APP_NAME),*)
...
.PHONY: app
app: reposync b2g_sdk profile-dir
	tar zxf profile_save.tar.gz; \
	if [ -e "apps" ] ; then \
...
.PHONY: reposync
reposync:
	@if [ ! -e "apps" -a -d "$(GAIA_DIR)/../apps" ] ; then \
		ln -sf $(GAIA_DIR)/../apps $(GAIA_DIR)/; \
	fi
...
really-clean: clean
	rm -rf b2g-* .b2g-* node_modules b2g modules.tar js-marionette-env "$(NODE_MODULES_CACHEDIR)"
