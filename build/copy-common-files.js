...
function execute(options) {
  ...
  // when prebuilt apps are used, need to read old webapps.json,
  // then update the app entry later.
  var manifestOldFile = webappsBaseDir.clone();
  manifestOldFile.append('webapps.json');
  var webappsJSON = utils.getJSON(manifestOldFile);
  ...
  gaia.webapps.forEach(function(app) {
    ...
    if (webappsJSON[webappTargetDirName]) {
        oldLocalId = webappsJSON[webappTargetDirName]['localId'];
    }
    webappsJSON[webappTargetDirName] = genWebappJSON(appConfig);
    if (oldLocalId != 0) {
        webappsJSON[webappTargetDirName]['localId'] = oldLocalId;
    }

    if (options.BUILD_APP_NAME === '*' &&
        webappTargetDirName.indexOf('kaios-plus') >= 0) {
      var settingsFile =
        utils.getFile(options.PROFILE_DIR, 'settings.json');
      var settings = utils.getJSON(settingsFile);
      var kaiOsPlusManifest = webappsJSON[webappTargetDirName];
      kaiOsPlusManifest.manifestURL =
