...
OperatorVariantHelper.prototype = {
  ...
  checkICCInfo: function() {
    ...
    var iccidChanged = (iccId !== this._iccSettings.iccId);
    if (mcc === '000') {
      return;
    }
        ...
        if (changed) {
          ...
          persistKeyGetRequest.onsuccess = (function persistKeyGetRequestCb() {
            ...
            if (!persistKeyGetRequest.result[this._persistKey]) {
              _applyNewApn();
            } else if (iccidChanged) {
              // If changed simcard but plmn & mvno don't change, need re set
              // apn value, because gecko listening 'ril.data.apnSettings'
              // change settings.
              let settingsLock = window.navigator.mozSettings.createLock();
              let request = settingsLock.get('ril.data.apnSettings');
              request.onsuccess = (() => {
                let result = request.result['ril.data.apnSettings'];
