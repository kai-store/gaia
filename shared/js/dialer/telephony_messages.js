...
/* globals ConfirmDialog, LazyLoader, ConfirmDialogHelper */
...
var TelephonyMessages = {
  ...
  showConfirmDialog: function( dialogType,
                               dialogTitle,
                               dialogBody,
                               dialogArgs,
                               softkeysL10nIds ) {
      ...
      var dialog = new ConfirmDialogHelper({
        type: dialogType,
        title: dialogTitle,
        body: { id: dialogBody, args: dialogArgs },
        accept: {
          title: 'OK',
          l10nId: softkeysL10nIds.accept
        }
      });
  ...
  showToast: function(options) {
    LazyLoader.load(['/shared/js/toaster.js'], function() {
  ...
  displayMessage: function(message, number) {
    ...
    this.showConfirmDialog( 'cd-type-msg-call-only',
                            dialogTitle,
                            dialogBody,
                            {number: number},
                            {accept: 'emergencyDialogBtnOk'} );
  },
  ...
  handleError: function(errorName, number, emergencyOptions) {
    ...
    if (errorName === 'BadNumberError') {
      if (emergencyOptions === this.NO_NETWORK) {
...
// TCL UI does not have this requirement
