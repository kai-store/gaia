...
var Contacts = {
  ...
  quickFindByNumber: function quickFindByNumber(number, callback) {
    this._findByNumber(number, callback, true);
  },

  _findByNumber: function _findByNumber(number, callback, quick) {
    if (!number) {
      callback(null);
      return;
    }
    ...
    request.onsuccess = function findCallback() {
      ...
      //find the full match index
      matches.forEach(function match(match, matchIndex) {
        match.forEach(function(matchNumber, localIndex) {
            if(number === matchNumber && matchResult == null){
              matchResult = {
      ...
      if(matchResult == null){
         matchResult = SimplePhoneMatcher.bestMatch(variants, matches);
      }
      ...
      if (!quick && fb.isFbLinked(contact)) {
    ...
    request.onerror = function findError() {
      console.log('Contacts: error ' + this.error.name);
      callback(null);
    };
  },
