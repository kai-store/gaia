...
(function(window) {
  ...
  window.SIMSlotManager = {
    ...
    _publishSIMSlotIfReady: function() {
      var numDetected = 0;
      this._instances.forEach(function iterator(slot) {
        if (!slot.isAbsent()) {
          numDetected++;
        }
      }, this);

      if (numDetected === this._instances.length) {
        this.publishSIMSlotIsReady();
        return;
      }

      if (!this._timerForSIM2 && this.isMultiSIM() && this.hasOnlyOneSIMCardDetected()) {
        this.waitForSecondSIM();
        return;
      }
    }
