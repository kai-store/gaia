...
(function(exports) {
   ...
   */
  function ContactToVcard(contacts, append, success, batchSize, skipPhoto, progressStep) {
    var vCardsString = '';
    var nextIndex = 0;
    var cardsInBatch = 0;
    ...
    function appendVCard(vcard) {
      ...
      if (typeof progressStep === 'function') {
        cancelled = progressStep();
      }
      ...
      if ((vCardsString.length > batchSize) ||
          (nextIndex === contacts.length) || cancelled) {
        append(vCardsString, cardsInBatch);
        cardsInBatch = 0;
        vCardsString = '';
      }

      if (nextIndex < contacts.length && !cancelled) {
